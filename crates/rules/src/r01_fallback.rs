...
impl Rule for FallbackMasksError {
    ...
    fn analyze(&self, ctx: &AnalysisContext) -> Result<Vec<Finding>, RuleError> {
        ...
        match ctx.language {
            ...
            Language::TypeScript | Language::TypeScriptTsx => find_catch_fallbacks(
                ctx,
                &mut findings,
                "catch 以默认返回值掩盖错误 | catch with default return masks error",
            ),
            Language::CSharp | Language::Java => find_catch_fallbacks(
                ctx,
                &mut findings,
